    ...
    public function index()
{
    ...
    // ADICIONADO: comerciais/staff ativos para usar no bulk assign
    ...
    public function save_longitude_and_latitude($client_id)
    {
        ...
        echo $this->db->affected_rows() > 0 ? 'success' : 'false';
    }
    ...
    public function form_contact($customer_id, $contact_id = '')
    {
        ...
        if (is_automatic_calling_codes_enabled()) {
            ...
            $callingCode = $clientCountry && $clientCountry->calling_code
                ? '+' . ltrim($clientCountry->calling_code, '+')
                : null;
        } else {
            $callingCode = null;
        }
        ...
        if ($this->input->post()) {
            ...
            if ($contact_id == '') {
                ...
                echo json_encode([
                    'success'             => $success,
                    'message'             => $message,
                    'has_primary_contact' => total_rows(db_prefix() . 'contacts', ['userid' => $customer_id, 'is_primary' => 1]) > 0,
    ...
    /**
     * Atualiza o status manual do cliente (coluna tblclients.status)
     * Aceita POST normal ou AJAX
     * Parâmetros:
     *  - id | customer_id (int)
    ...
    public function update_assigned()
{
    ...
    if (!$userid) {
        return $this->output
            ...
            ->set_output(json_encode(['success' => false, 'error' => 'missing_userid']));
    }

    // Se não for admin, só pode atribuir-se a si próprio
    if (!is_admin() && $assigned !== get_staff_user_id()) {
        return $this->output
            ->set_content_type('application/json')
            ->set_output(json_encode(['success' => false, 'error' => 'not_allowed']));
    }

    $this->db->where('userid', $userid);
    $ok = $this->db->update(db_prefix() . 'clients', [
        'assigned' => $assigned,
    ]);
    ...
    if ($ok && !$dbError['code']) {
        log_activity('Customer assigned updated [ID: ' . $userid . ' -> StaffID: ' . $assigned . ']');
        hooks()->do_action('customer_manual_assigned_updated', [
            'id'       => $userid,
            'assigned' => $assigned,
        ]);
    }
        ...
        public function bulk_action()
    {
        ...
        if ($this->input->post()) {
            ...
            $assigned = $this->input->post('assigned'); // <-- novo campo para comercial
            ...
            if (is_array($ids)) {
                foreach ($ids as $id) {
                    ...
                    if ($this->input->post('mass_delete')) {
                        ...
                        // Ação: Atualizar "assigned" (atribuir comercial)
                        if ($assigned) {
                            $this->db->where('userid', $id);
                            $ok = $this->db->update(db_prefix() . 'clients', ['assigned' => (int) $assigned]);
                            if ($ok) {
                                $total_updated++;
                            }
                        }
        ...
        if ($this->input->post('mass_delete')) {
            ...
            set_alert('success', $total_updated . ' clientes atribuídos ao comercial selecionado.');
        } else {
            set_alert('success', _l('updated_successfully', _l('client')));
        }
    }
        ...
        public function update_status()
{
    ...
    // Atualização direta — evita check_zero_columns() do clients_model->update()
    $this->db->where('userid', $userid);
    $ok = $this->db->update(db_prefix() . 'clients', ['status' => $status]);

    $dbError = $this->db->error();
    $rows    = $this->db->affected_rows();

    if ($ok && !$dbError['code']) {
        log_activity('Customer manual status updated [ID: ' . $userid . ' -> Status: ' . $status . ']');
        hooks()->do_action('customer_manual_status_updated', [
            'id'     => $userid,
            'status' => $status,
        ]);
    }

    // Mesmo que não haja linhas afetadas (status igual), considera sucesso
    return $this->output
        ->set_content_type('application/json')
        ->set_output(json_encode([
            'success'       => (bool) $ok && !$dbError['code'],
            'message'       => $ok ? 'Status atualizado' : 'Falha ao atualizar',
            'id'            => $userid,
            'status'        => $status,
            'rows_affected' => $rows,
            'db_error'      => $dbError,
