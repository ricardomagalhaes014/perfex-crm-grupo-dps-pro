                            ...
                            <div class="modal-dialog" role="document">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        ...
                                        <!-- NOVO: ATRIBUIR COMERCIAL (sem remover nada do existente) -->
                                        <div id="bulk_assign" class="mtop15">
                                            <div class="form-group">
                                                <label for="bulk_assigned"><?= _l('atribuir_comercial'); ?></label>
                                             <select id="bulk_assigned" name="bulk_assigned" class="selectpicker" 
                                                ...
                                                foreach ($staff as $m) {
                                                    echo '<option value="' . html_escape($m['staffid']) . '">' 
                                                         . html_escape($m['firstname'] . ' ' . $m['lastname']) 
                                                         . '</option>';
                                                }
                                                ?>
                                            </select>
...
<script>
    ...
    function customers_bulk_action(event) {
        ...
        if (r == false) {
            ...
            if (mass_delete == false || typeof(mass_delete) == 'undefined') {
                ...
                // NOVO: comercial escolhido
                var assigned = $('#bulk_assigned').val();
                if (assigned) {
    ...
    // Atualiza o status via AJAX quando o select muda
    $(document).on('change', '.client-status-select', function () {
        var id  = $(this).data('id');
        var val = $(this).val();
        $.post(admin_url + 'clients/update_status', { id: id, status: val })
            .done(function () { alert_float('success', app.lang.updated_successfully || 'Atualizado'); })
            .fail(function () { alert_float('danger', 'Erro ao guardar o status'); });
    });
</script>
</body>
