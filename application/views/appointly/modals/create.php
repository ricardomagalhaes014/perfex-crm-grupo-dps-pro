  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <?php echo form_open('admin/appointments/create', ['id' => 'appointment_form']); ?>
...
$(function () {
  $('#search_input').on('input', function () {
    ...
    $.post(admin_url + 'appointments/search_clients', { q: query, type: type }, function (res) {
      const results = $('#search_results').empty().show();
      if (res.length > 0) {
        res.forEach(item => {
          results.append(`<a href="#" class="dropdown-item" data-id="${item.id}">${item.label}</a>`);
        });
      } else {
