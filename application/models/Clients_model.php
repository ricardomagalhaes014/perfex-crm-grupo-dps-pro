...
class Clients_model extends App_Model
{
    ...
    */
    ...
    public function get_contacts_for_project_notifications($projectId, $type)
    {
        ...
        /**
     * Atualiza o STATUS do cliente (campo custom)
     * @param int $id
     * @param string $status
     * @return bool
     */
    public function update_status($id, $status)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            log_activity('Customer Status Updated [ID: ' . $id . ' | Status: ' . $status . ']');
            hooks()->do_action('client_status_custom_updated', ['id' => $id, 'status' => $status]);
            return true;
        }
    ...
    /**
     * Atualiza o STAFF atribuído ao cliente (campo custom)
     * @param int $id
     * @param int|null $staff_id
     * @return bool
     */
    public function update_assigned($id, $staff_id = null)
    {
        $this->db->where('userid', $id);
        $this->db->update(db_prefix() . 'clients', [
            'assigned' => $staff_id,
        ...
        if ($this->db->affected_rows() > 0) {
            log_activity('Customer Assigned Updated [ID: ' . $id . ' | Assigned: ' . $staff_id . ']');
            hooks()->do_action('client_assigned_updated', ['id' => $id, 'assigned' => $staff_id]);
            return true;
        }
        ...
        /**
     * Regras de filtro para tabela de clientes
     * (usado pelo <app-filters>)
     */
    public function get_rules()
    {
        $rules = [];

        // Exemplo de regra existente (empresa)
        $rules[] = [
            'id'    => 'company',
            'field' => db_prefix() . 'clients.company',
        ...
        $rules[] = [
            'id'     => 'status',
            'field'  => db_prefix() . 'clients.status',
            'label'  => _l('status'),
            'type'   => 'select',
            'values' => [
