...
/**
 * AURA Lead Endpoint
 * Recebe leads do Make (Facebook Lead Ads - Formulário AURA)
 * Cria lead no Perfex CRM com tag AURA, atribuída ao Ricardo (staff_id=1)
...
// Token de segurança
define('AURA_TOKEN', 'dps-aura-2026');

// Configurar logging
$log_file = __DIR__ . '/aura-lead-debug.log';
...
try {
    $stmt = $pdo->prepare("
        ...
        VALUES
            (?, ?, ?, ?, ?, ?, ?, NOW(), NULL, 1, 0, NULL)
    ");
    $description = "Lead Facebook Lead Ads - Formulário AURA\nFacebook Lead ID: " . $fb_lead_id;
    $stmt->execute([
        $status_id,
        $source_id,
        $assigned,
...
try {
    ...
    log_debug("Tag AURA associada com sucesso");
} catch (PDOException $e) {
    // Não falhar por causa da tag
    log_debug("Erro ao associar tag AURA", $e->getMessage());
}
...
if (!empty($phone)) {
    try {
        ...
        if (!empty($evo_url) && !empty($evo_key)) {
            ...
            if ($is_connected) {
                // Preparar mensagem de boas-vindas para AURA
