<?php
// Patch minimalista - apenas atualiza a view do Sofia Calls
$view_path = dirname(__FILE__) . '/modules/dps_sofia_calls/views/sofia_calls/index.php';
$content = file_get_contents('https://raw.githubusercontent.com/ricardomagalhaes014/perfex-crm-grupo-dps-pro/main/modules/dps_sofia_calls/views/sofia_calls/index.php');
if($content && strlen($content) > 1000) {
    file_put_contents($view_path, $content);
    echo json_encode(['status'=>'OK','bytes'=>strlen($content),'path'=>$view_path]);
} else {
...
PHPEOF;
    file_put_contents($view_path, $direct);
    echo json_encode(['status'=>'OK_direct','bytes'=>strlen($direct)]);
}
