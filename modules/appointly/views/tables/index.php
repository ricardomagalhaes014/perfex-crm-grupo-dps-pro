...
if (!is_admin() && !staff_appointments_responsible()) {
    array_push($where, 'AND (' . db_prefix() . 'appointly_appointments.created_by=' . get_staff_user_id() . '
        OR ' . db_prefix() . 'appointly_appointments.id IN (
...
$filterMap = [
    ...
    'lead_related'    => 'source = "lead_related"',
    'internal_staff'  => 'source = "internal_staff_crm"',
    'not_approved'    => 'approved != 1',
    'upcoming'        => 'date > CURDATE()',
    'missed'          => 'date < CURDATE()',
    'recurring'       => 'recurring = 1'
];

foreach ($filterMap as $input => $condition) {
    if ($this->ci->input->post($input)) {
...
foreach ($rResult as $aRow) {
    ...
    $can_approve = ($aRow['approved'] == 0 && $aRow['cancelled'] == 0) &&
    ...
    if (staff_can('edit', 'appointments') || staff_appointments_responsible()) {
        $editFn = ($aRow['source'] != 'internal_staff_crm') ? 'appointmentUpdateModal' : 'appointmentGlobalStaffModal';
        $nameRow .= ' | <a href="#" data-toggle="tooltip" title="' . _l('appointment_edit_meeting') . '" data-id="' . $aRow['id'] . '" onclick="' . $editFn . '(this); return false;">' . _l('edit') . '</a>';
    }

    $isContact = ($aRow['contact_id']) ? 0 : 1;

    // Criar tarefa a partir do agendamento
    if (staff_can('create', 'tasks') && $aRow['approved'] == 1 && $aRow['source'] != 'internal_staff_crm') {
        $nameRow .= ' | <a data-toggle="tooltip" title="' . _l('appointments_create_task_tooltip') . '" href="#" 
            data-customer-id="' . appointly_get_contact_customer_id($aRow['contact_id']) . '" 
            data-source="' . $aRow['source'] . '" 
            data-contact-id="' . $aRow['contact_id'] . '" 
            data-name="' . $aRow['name'] . '" 
            onclick="new_task_from_relation_appointment(this); return false;">' . _l('new_task') . '</a>';
    }

    // Converter para lead
    if ($isContact && $aRow['approved'] == 1 && $aRow['source'] != 'internal_staff_crm') {
        $nameRow .= ' | <a data-toggle="tooltip" title="' . _l('appointments_convert_to_lead_tooltip') . '" 
            href="#" data-name="' . $aRow['name'] . '" 
            data-email="' . $aRow['contact_email'] . '" 
            data-phone="' . $aRow['phone'] . '" 
            onclick="init_appointment_lead(this);return false;">' . _l("appointments_convert_to_lead_label") . '</a>';
    }
        // Feedback e finalizar
    if ($aRow['feedback'] !== null && $aRow['finished'] !== 1) {
        $nameRow .= ' | <a data-toggle="tooltip" title="' . _l('appointment_view_feedback') . '" href="' . admin_url('appointly/appointments/view?appointment_id=' . $aRow['id']) . '#feedback_wrapper">' . _l('appointment_view_feedback') . '</a>';
    } elseif ($aRow['finished'] == 1) {
    ...
    // Data formatada com status
    $row[] = '<span ' . $tooltip . ' class="label label-' . $label_class . '">' . _dt($aRow['date']) . '</span>';

    // Criador do agendamento (staff ou cliente)
    if ($aRow['creator_firstname']) {
        $staff_fullname = $aRow['creator_firstname'] . ' ' . $aRow['creator_lastname'];
        $row[] = '<a class="initiated_by" target="_blank" href="' . admin_url("profile/" . $aRow["created_by"]) . '">
                    <img src="' . staff_profile_image_url($aRow["created_by"], "small") . '" data-toggle="tooltip" data-title="' . $staff_fullname . '" class="staff-profile-image-small mright5">' . $staff_fullname . '
                  </a>';
    } else {
        $row[] = $aRow['name'];
    }
    ...
    $row[] = $aRow['description'];
        ...
        $canApprove = $aRow['approved'] == 0 && $aRow['cancelled'] == 0 && (is_admin() || staff_can('view', 'appointments'));
        if ($canApprove) {
            $outputStatus .= '<li><a href="#" onclick="markAppointmentAsApproved(' . $aRow['id'] . '); return false">' . _l('task_mark_as', 'Approved') . '</a></li>';
        }

        if ($aRow['cancelled'] == 0 && $aRow['finished'] == 0 && ($aRow['created_by'] == get_staff_user_id() || staff_appointments_responsible())) {
            $outputStatus .= '<li><a href="#" onclick="markAppointmentAsCancelled(' . $aRow['id'] . '); return false">' . _l('task_mark_as', 'Cancelled') . '</a></li>';
        }

        if ($aRow['finished'] == 0 && $aRow['cancelled'] == 0 && $aRow['approved'] != 0 && ($aRow['created_by'] == get_staff_user_id() || staff_appointments_responsible())) {
            $outputStatus .= '<li><a href="#" onclick="markAppointmentAsFinished(' . $aRow['id'] . '); return false">' . _l('task_mark_as', 'Finished') . '</a></li>';
        }

        if ($aRow['cancelled'] == 1 && $aRow['finished'] == 0 && ($aRow['created_by'] == get_staff_user_id() || staff_appointments_responsible())) {
            $outputStatus .= '<li><a href="#" onclick="markAppointmentAsOngoing(' . $aRow['id'] . '); return false">' . _l('task_mark_as', 'Ongoing') . '</a></li>';
        }

        $outputStatus .= '</ul></div>';
    } else {
        $currentStatus = checkAppointlyStatus($aRow);
        $outputStatus = '<div><a data-toggle="tooltip" title="' . $currentStatus . '" href="#" style="font-size:14px;cursor:context-menu;" class="text-dark"><span class="label label-callback-status-' . $currentStatus . '">' . $currentStatus . '</span></a></div>';
    }
    ...
    switch ($aRow['source']) {
        ...
        case 'internal_staff_crm':
            ...
            break;
        default:
            $row[] = ucfirst($aRow['source']);
    }

    // Links de calendário
    $options = '<div class="text-center">';
    $_google_calendar_link = !empty($aRow['google_calendar_link']) && $aRow['google_added_by_id'] == get_staff_user_id();
    $_outlook_calendar_link = !empty($aRow['outlook_calendar_link']) && $aRow['outlook_added_by_id'] == get_staff_user_id();
    ...
    if (!$_google_calendar_link && !$_outlook_calendar_link) {
        $options .= '<p class="text-muted">' . _l('appointments_not_added_to_calendar') . '</p>';
    }

    $options .= '</div>';
        // ID para a linha
    $row['DT_RowId'] = 'appointment_id_' . $aRow['id'];

    // Classe para habilitar ações extras
    $row['DT_RowClass'] = 'has-row-options';

    // Adiciona a coluna de opções (última)
    $row[] = $options;

    // Adiciona a linha ao array final
    $output['aaData'][] = $row;
}