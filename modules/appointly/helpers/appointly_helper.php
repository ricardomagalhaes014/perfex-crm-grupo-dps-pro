...
if ( ! function_exists('appointly_get_staff_customers')) {
    function appointly_get_staff_customers()
    {
        ...
        $CI->db->select('firstname, lastname, vat, '.db_prefix().'contacts.id as contact_id, '.get_sql_select_client_company());
        ...
        foreach ($result as &$contact) {
            if ($contact['company']==$contact['firstname'].' '.$contact['lastname']) {
                ...
                $contact['company'] = ""._l('appointments_company_for_select')."(".$contact['company']." | NIF ".$contact['vat'].")";
            }
        }
...
if (!function_exists('fetch_appointment_data')) {
    function fetch_appointment_data($appointment_id)
    {
        ...
        if (!empty($appointment)) {
            ...
            // Buscar os participantes da marcação
            $CI->load->model('appointly/appointly_attendees_model', 'atm');
            $appointment['attendees'] = $CI->atm->attendees($appointment_id);

            $appointment['selected_staff'] = array_map(function ($staff_id) {
                return $staff_id;
...
if (!function_exists('appointly_add_settings_tab')) {
    function appointly_add_settings_tab()
    {
        if (is_admin()) {
            get_instance()->app->add_settings_section('appointly', [
