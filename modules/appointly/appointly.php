...
// Registar campos de mesclagem adicionais
hooks()->add_filter('other_merge_fields_available_for', function ($fields) {
    return appointly_register_other_merge_fields($fields);
});

// Permitir campos de staff em templates de marcações
hooks()->add_filter('available_merge_fields', function ($fields) {
    return appointly_allow_staff_merge_fields_for_appointment_templates($fields);
});

// Adicionar widgets personalizados ao dashboard
hooks()->add_filter('get_dashboard_widgets', function ($widgets) {
    return appointly_register_dashboard_widgets($widgets);
});

// Injetar marcações no calendário (com prioridade e argumentos definidos)
hooks()->add_filter('calendar_data', function ($data, $type) {
    return appointly_register_appointments_on_calendar($data, $type);
}, 10, 2);
...
if (!function_exists('appointly_add_settings_section')) {
    function appointly_add_settings_section()
    {
        $CI =& get_instance();

        if (isset($CI->app) && method_exists($CI->app, 'add_settings_section')) {
            $CI->app->add_settings_section('appointments', [
                'name'     => _l('appointments'),
