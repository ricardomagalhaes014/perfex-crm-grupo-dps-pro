...
class Mailbox_model extends App_Model
{
    ...
    public function add($data, $staff_id, $ob_id = null)
    {
        ...
        if (is_array($array_to) && count($array_to) > 0) {
            foreach ($array_to as $value) {
                $array_send_to[$value] = $value;
            }
        }
        $array_cc = explode(';', $data['cc']);
        if (is_array($array_cc) && count($array_cc) > 0) {
        ...
        $array_inbox_id = [];
        if (is_array($array_send_to) && count($array_send_to) > 0) {
        ...
        $attachments = [];
        if ($outbox_id > 0) {
            if (is_array($array_inbox_id) && count($array_inbox_id) > 0) {
    ...
    public function conversationTicket($data){
        if (isset($data['select_ticket']) && is_array($data['select_ticket'])) {
