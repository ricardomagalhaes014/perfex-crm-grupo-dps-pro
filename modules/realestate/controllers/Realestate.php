	...
	public function add_edit_property_listing($id = '')
{
    ...
    if ($this->input->post()) {
        ...
        if ($id == '') {
            ...
            $data['formdata'][] = ['name' => 'is_company_admin','value' => $check_staff_type['is_company_admin']];
            $data['formdata'][] = ['name' => 'company_id','value' => $check_staff_type['company_id']];
            $data['formdata'][] = ['name' => 'broker_id','value' => $broker_id];
            $data['formdata'][] = ['name' => 'related_type','value' => $check_staff_type['staff_type']];
            $data['formdata'][] = ['name' => 'related_id','value' => $related_id];

            $insert_id = $this->realestate_model->add_property_listing($data);
            $url       = admin_url('realestate/properties');

            if ($insert_id) {
                $property_listing = $this->realestate_model->get_property_listing($insert_id);
                $nome = !empty($property_listing->description) ? trim($property_listing->description) : "Imóvel {$insert_id}";
                $descricao = (isset($property_listing->long_description) && $property_listing->long_description !== '') ? $property_listing->long_description : ($property_listing->description ?? '');
                $resumo    = $property_listing->short_description ?? '';
                $imagem = '';
                $assets = $this->realestate_model->get_property_asset($insert_id);                
				if (!empty($assets)) {
    				foreach ($assets as $asset) {
        			// Verifica se a chave 'site_url' existe e não está vazia
        				if (!empty($asset['site_url'])) {
            			// A URL já está pronta, basta usá-la
            				$imagem = $asset['site_url'];
            				break; // Pega a primeira imagem e para o loop
        				}
    				}
				}
                ...
                $propertyData = [
                    'id'                 => $id, 'nome' => $nome, 'descricao' => $descricao, 'resumo' => $resumo,
                    'preco'              => $property_listing->rate ?? '', 'tipo_anuncio' => $property_listing->listing_type ?? '',
                    'estilo_imovel'      => $property_listing->property_style ?? '', 'cidade' => $property_listing->city ?? '',
                    'pais'               => $property_listing->country ?? '', 'banheiros_completos' => $property_listing->full_baths ?? '',
                    'status'             => $property_listing->status ?? '', 'tamanho_lote_acres' => $property_listing->lot_size_acres ?? '',
                    'ativo'              => 1,
                ];
                
                
                if (!empty($imagem)) { $propertyData['imagem'] = $imagem; }

                $exportUrl = "https://dpsimobiliario.grupo-dps.com/wp-json/imoveis/v1/receber";
                $logFile = __DIR__ . "/sync_debug.log";
                file_put_contents($logFile, date('Y-m-d H:i:s' ) . " ENVIANDO PARA WP: " . json_encode($propertyData) . "\n", FILE_APPEND);

                $ch = curl_init($exportUrl);
                curl_setopt_array($ch, [CURLOPT_POST => true, CURLOPT_CONNECTTIMEOUT => 15, CURLOPT_TIMEOUT => 30, CURLOPT_RETURNTRANSFER => true, CURLOPT_HTTPHEADER => ['Content-Type: application/json'], CURLOPT_POSTFIELDS => json_encode($propertyData)]);
                $response = curl_exec($ch);
                $http     = curl_getinfo($ch, CURLINFO_HTTP_CODE );
                $err      = curl_error($ch);
                curl_close($ch);

                file_put_contents($logFile, date('Y-m-d H:i:s') . " RESPOSTA WP: HTTP {$http} | ERR: {$err} | " . $response . "\n", FILE_APPEND );
                if ($response === false) { log_message('error', 'SYNC_IMOVEIS cURL error: ' . $err); } else { log_message('debug', 'SYNC_IMOVEIS response: ' . $response); }
                
                set_alert('success', _l('added_successfully', _l('real_listing')));
                echo json_encode(['url' => $url, 'commodityid' => $insert_id, 'add_or_update' => 'add']);
                die;
            }

            set_alert('warning', _l('mrp_added_failed'));
            echo json_encode(['url' => $url, 'add_or_update' => 'add']);
            die;

        } else {
            // Bloco para EDITAR imóvel existente
            ...
            if ($success) {
                ...
                $nome = !empty($property_listing->description) ? trim($property_listing->description) : "Imóvel {$id}";
                $descricao = (isset($property_listing->long_description) && $property_listing->long_description !== '') ? $property_listing->long_description : ($property_listing->description ?? '');
                $resumo    = $property_listing->short_description ?? '';
                $imagem = '';
                $assets = $this->realestate_model->get_property_asset($id);
				if (!empty($assets)) {
    				foreach ($assets as $asset) {
        			// Verifica se a chave 'site_url' existe e não está vazia
        				if (!empty($asset['site_url'])) {
                ...
                $exportUrl = "https://dpsimobiliario.grupo-dps.com/wp-json/imoveis/v1/receber";
                $logFile = __DIR__ . "/sync_debug.log";
                file_put_contents($logFile, date('Y-m-d H:i:s' ) . " ENVIANDO UPDATE PARA WP: " . json_encode($propertyData) . "\n", FILE_APPEND);
                ...
                file_put_contents($logFile, date('Y-m-d H:i:s') . " RESPOSTA UPDATE WP: HTTP {$http} | ERR: {$err} | " . $response . "\n", FILE_APPEND );
                if ($response === false) { log_message('error', 'SYNC_IMOVEIS UPDATE cURL error: ' . $err); } else { log_message('debug', 'SYNC_IMOVEIS UPDATE response: ' . $response); }
                
                set_alert('success', _l('updated_successfully', _l('real_listing')));
                $url = admin_url('realestate/properties');
                echo json_encode(['url' => $url, 'commodityid' => $id, 'add_or_update' => 'update']);
                die;
            }
        }
    }

    // ----- Parte que carrega o formulário de add/edit -----
    if ($id == '') {
        ...
        if (isset($property_listing->listing_rooms) && count($property_listing->listing_rooms) > 0) {
            ...
            foreach ($property_listing->listing_rooms as $listing_room) {
                $index_receipt++;
                $property_listing_room .= $this->realestate_model->create_property_listing_room_row_template(
                    [], [], 'items[' . $index_receipt . ']', $listing_room['room_type'],
                    $listing_room['rooms_level'], $listing_room['room_demension_width'],
                    $listing_room['room_demension_lenght'], $listing_room['room_benefits'],
                    $listing_room['id'], true
                );
            }
        }
