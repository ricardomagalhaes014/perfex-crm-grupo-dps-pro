...
class Si_lead_filters extends AdminController 
{
	...
	/**
	 * Página: Interacções por Comercial
	 * Mostra o total de interacções (notas manuais) por comercial no período seleccionado,
	 * com filtro de período e status, e detalhe das leads com interacção.
	 */
	public function interacoes_comercial()
	{
		$p = db_prefix();
		...
		// Período por defeito: últimos 7 dias
		if (empty($periodo)) $periodo = 'last_7';
		...
		$status_clause = '';
		if ($status_id > 0) {
			$status_clause = "AND l.status = {$status_id}";
		}

		// ── Obter todos os staff activos ──────────────────────────────────────
		$staff_list = $this->db
			->select('staffid, CONCAT(firstname, " ", lastname) as full_name')
			->where('active', 1)
			->order_by('firstname', 'asc')
		...
		foreach ($staff_list as $staff) {
			...
			// Leads atribuídas a este comercial (com filtro de status se aplicável)
			...
			// Contar interacções (notas manuais) no período para as leads deste comercial
			$int_sql = "SELECT leadid, COUNT(*) as cnt
						...
						AND date <= '{$date_to}'
						GROUP BY leadid";
			$int_result = $this->db->query($int_sql);
			$int_map = [];
			if ($int_result) {
				foreach ($int_result->result_array() as $row) {
					$int_map[(int)$row['leadid']] = (int)$row['cnt'];
				}
			}

			// Associar interacções às leads e filtrar só as que têm interacções
			$leads_com_interacao = [];
			$total = 0;
			foreach ($leads_raw as $lead) {
				$cnt = isset($int_map[(int)$lead['id']]) ? $int_map[(int)$lead['id']] : 0;
				$total += $cnt;
				if ($cnt > 0) {
					$lead['interactions'] = $cnt;
					$leads_com_interacao[] = $lead;
				}
			}

			// Ordenar leads por número de interacções (desc)
			usort($leads_com_interacao, function($a, $b) {
			...
			$comerciais[] = [
				...
				'leads'            => $leads_com_interacao,
			];
		}

		// ── Ordenar comerciais por total de interacções (desc) ────────────────
		...
		$this->load->view('interacoes_comercial', $data);
	}
