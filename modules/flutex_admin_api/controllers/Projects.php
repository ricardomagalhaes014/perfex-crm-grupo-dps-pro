...
class Projects extends RestController
{
    ...
    public function projects_get()
    {
        ...
        if (!empty($projectData) && !empty($projectID)) {
            $projectData->total_expenses               = sum_from_table(db_prefix() . 'expenses', ['where' => ['project_id' => $projectID], 'field' => 'amount']);
            $projectData->billable_expenses            = sum_from_table(db_prefix() . 'expenses', ['where' => ['project_id' => $projectID, 'billable' => 1], 'field' => 'amount']);
            $projectData->billed_expenses              = sum_from_table(db_prefix() . 'expenses', ['where' => ['project_id' => $projectID, 'invoiceid !=' => 'NULL', 'billable' => 1], 'field' => 'amount']);
            $projectData->unbilled_expenses            = sum_from_table(db_prefix() . 'expenses', ['where' => ['project_id' => $projectID, 'invoiceid IS NULL', 'billable' => 1], 'field' => 'amount']);
            $projectData->total_tasks                  = total_rows(db_prefix() . 'tasks', 'rel_type = "project" AND rel_id='.$projectID);
            $projectData->open_tasks                   = total_rows(db_prefix() . 'tasks', 'rel_type = "project" AND rel_id='.$projectID. ' AND ' . 'status != ' . Tasks_model::STATUS_COMPLETE);
