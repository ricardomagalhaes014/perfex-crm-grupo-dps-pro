...
class Tasks extends RestController
{
    ...
    public function tasks_get()
    {
        ...
        $taskData = $this->tasks_api_model->get($where,$limit,$offset,$sort);
        $task_summary = $this->tasks_summary();
        
        if (!empty($taskData)) {
            $this->response(['message' => _l('data_retrieved_successfully'), 'data' => $taskData, 'overview' => $task_summary], RestController::HTTP_OK);
        } else {
            $this->response(['message' => _l('data_not_found'), 'overview' => $task_summary], RestController::HTTP_NOT_FOUND);
        }
    }
