(function(){
  ...
  $('.sync_products_from_woo').click(function(){
    var omni_warehouse = $('select[name="omni_warehouse"]').val();
    if(omni_warehouse != ""){
      $('.status-sync').removeClass('label-primary');
      $('.status-sync').addClass('label-danger');
      $('.status-sync').text('Wait for sync');
      ...
      $.post(admin_url+'omni_sales/process_asynclibrary_info_full/'+id+'/'+omni_warehouse).done(function(response){
        $('.status-sync').removeClass('label-danger');
        $('.status-sync').addClass('label-success');
        $('.status-sync').text('Sync success');
        $('#box-loadding').html('');
        ...
        alert_float('warning', 'Please select a warehouse to complete the sync!');
    }
  })
