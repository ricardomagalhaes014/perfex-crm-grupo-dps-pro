	...
	public function __construct()
	{
		...
		public function order_list($tab = ''){
			...
			if(is_numeric($userid)){
				$data['cart_list'] = $this->omni_sales_model->get_cart_of_client_by_status($userid,$tab,'', '(channel_id = 2 OR channel_id = 6)');
			}
			$this->data($data);
 ...
 * @return json         
 */
public function get_order_list($status, $type, $ofset, $limit){
	$where = '';
	if($type == -1){
		$type = '';
		$where = '(channel_id = 2 OR channel_id = 6)';
	}
	$userid = get_client_user_id();
	$html = '';
	if(is_numeric($userid)){
