<?php
/**
 * SAMARA Lead Endpoint
 * Recebe leads do Make (Facebook Lead Ads - Formulário Samara)
 * Cria lead no Perfex CRM com tag SAMARA, atribuída à Samara (staff_id=17)
 * Envia mensagem de WhatsApp de boas-vindas
 */
define('BASEPATH', __DIR__ . '/');
define('SAMARA_TOKEN', 'dps-samara-2026');
...
$assigned   = 17; // Samara Ferreira (staff_id fixo)
...
if (!empty($email) || !empty($phone)) {
    ...
    $check_params = [];
    if (!empty($email)) { $check_sql .= " OR email = ?"; $check_params[] = $email; }
    if (!empty($phone)) { $check_sql .= " OR phonenumber = ?"; $check_params[] = $phone; }
...
try {
    $stmt = $pdo->prepare("
        ...
        VALUES
            (?, ?, ?, ?, ?, ?, ?, NOW(), NULL, 1, 0, NULL)
    ");
    $description = "Lead Facebook Lead Ads - SAMARA\nFacebook Lead ID: " . $fb_lead_id;
    $stmt->execute([
        $status_id,
        $source_id,
        $assigned,
...
// Adicionar tag SAMARA
try {
    $stmt = $pdo->prepare("SELECT id FROM tbltags WHERE name = 'SAMARA' LIMIT 1");
    $stmt->execute();
    $tag_row = $stmt->fetch(PDO::FETCH_ASSOC);
    if ($tag_row) {
        $tag_id = $tag_row['id'];
    } else {
        $pdo->prepare("INSERT INTO tbltags (name) VALUES ('SAMARA')")->execute();
        $tag_id = $pdo->lastInsertId();
        log_debug("Tag SAMARA criada com id=" . $tag_id);
    }
    $pdo->prepare("INSERT IGNORE INTO tbltaggables (tag_id, rel_id, rel_type, tag_order) VALUES (?, ?, 'lead', 0)")
        ->execute([$tag_id, $lead_id]);
    log_debug("Tag SAMARA adicionada à lead");
} catch (PDOException $e) {
    log_debug("Erro ao adicionar tag SAMARA", $e->getMessage());
}
    ...
    try {
        ...
        if (!empty($evo_url) && !empty($evo_key)) {
            ...
            if ($is_connected) {
                ...
                curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode(['number' => $wa_number, 'text' => $message]));
                ...
                if ($http_code >= 200 && $http_code < 300) {
                    ...
                    $stmt_note->execute([$lead_id, $assigned, '[WhatsApp Automático] Mensagem de boas-vindas enviada para ' . $wa_number]);
